...
if [ -f /var/www/html/404.html ]; then
    ...
    ls -lh /var/www/html/404.html
else
    echo "✗ 404.html NOT FOUND - creating it now..."
    sudo cp 5-design_a_beautiful_404_page.html /var/www/html/404.html 2>/dev/null || \
...
echo "Looking for error_page directive in default config..."
if sudo grep -q "error_page 404" /etc/nginx/sites-enabled/default; then
    echo "✓ error_page 404 directive found"
    sudo grep "error_page 404" /etc/nginx/sites-enabled/default
else
    echo "✗ error_page 404 directive NOT FOUND - adding it now..."
    # Show current server block
    echo "Current server block:"
    sudo grep -A 3 "server_name" /etc/nginx/sites-enabled/default | head -5
    
    # Add the directive properly - backup first
    sudo cp /etc/nginx/sites-enabled/default /etc/nginx/sites-enabled/default.backup
    
    # Insert error_page directive properly
    sudo sed -i '/^[[:space:]]*server_name.*;$/a\    error_page 404 /404.html;' /etc/nginx/sites-enabled/default
fi
...
echo ""
echo "✓ All checks complete! Try accessing your domain again."
echo "Example: http://skeza.tech/nonexistent"
